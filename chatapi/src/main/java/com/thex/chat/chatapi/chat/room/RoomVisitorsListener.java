...
public class RoomVisitorsListener {
    ...
    public void onRoomVisitors(RoomVisitors message) {
        ServerSession session = bayeuxServer.getSession(message.targetConnectionId());
        if (session == null) {
            log.info("No cometd session {} to deliver visitors for room {}",
                message.targetConnectionId(), message.roomId());
            return;
        }
